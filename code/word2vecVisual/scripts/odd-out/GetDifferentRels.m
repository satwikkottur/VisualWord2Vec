...
preFile = fullfile(modelPath, 'wiki_iters/word2vec_wiki_pre_0_0_1_25.txt');
postFile = fullfile(modelPath, 'wiki_iters/word2vec_wiki_post_0_0_1_25.txt');
...
simPre = 1 - pdist2(uniqPreR, uniqPreR, 'cosine');
simPost = 1 - pdist2(uniqPostR, uniqPostR, 'cosine');
...
% simPost(A, B) < simPost(A, C)
maskC = (simPre <= lowerThresh) & (simPost >= upperThresh);
maskB = (simPost <= lowerThresh) & (simPre >= upperThresh);
for i = 1:noR
    rInds = find(maskC(i, :));
    % For each element in mask, find other relations
    for k = rInds
        otherR = (simPre(i, k) < simPre(i, :)) & ...
                    (simPost(i, k) > simPost(i, :));

        %Check only the extremes
...
%    % For each element in mask, find other relations
%    for j = rInds
%        otherR = (simPre(i, :) < simPre(i, j)) & ...
%                    (simPost(i, :) > simPost(i, j));
%        for k = find(otherR)
...
% Isolate relations w2v thinks are similar
% simPre(A, B) > simPre(A, C)
% simPost(A, B) < simPost(A, C)
maskB = (simPre <= lowerThresh) & (simPost >= upperThresh);
maskC = (simPost <= lowerThresh) & (simPre >= upperThresh);
for i = 1:noR
    rInds = find(maskC(i, :));
    % For each element in mask, find other relations
    for k = rInds
