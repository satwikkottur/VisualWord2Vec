...
[uniqR, ~, mapR] = unique(Rlabel);
noR = length(uniqR);
cooccur = zeros(noR, noR);

for i = 1:noInst
    curId = cIds(i, 1);
    for j = i+1:noInst
        if(cIds(j, 1) == curId && ~strcmp(Rlabel{i}, Rlabel{j}))
            cooccur(mapR(i), mapR(j)) = cooccur(mapR(i), mapR(j)) + 1;
            cooccur(mapR(j), mapR(i)) = cooccur(mapR(j), mapR(i)) + 1;
        end
    end
end
...
topR = cell(topN, 3);
for i = 1:topN
    % Pull out top 10 values
    [maxVal, maxInd] = max(cooccur(:));
    if(maxVal == -Inf)
        print 'Error'
    end
    [row, col] = ind2sub(size(cooccur), maxInd);
    topR(i, :) = {uniqR{row}, uniqR{col}, num2str(cooccur(row, col))};
    cooccur(row, col) = -Inf;
    cooccur(col, row) = -Inf;
end

save('Co-occurance.mat', 'cooccur', 'uniqR', 'topR');
%topR
