 #!/usr/bin/python
 # -*- coding: utf-8 -*-
from shadow_render import makeShadow
import json
...
class RenderScenes(object):
    ...
    def render_one_scene(self, data, primary_object=-1, primary_instance=-1, secondary_object=-1, secondary_instance=-1):
                        ...
                        for i in range(0, len(cur_avail_obj)):
                            if (cur_avail_obj[i]['instance'][0]['depth0'] == k):
                                for m in range(0, cur_avail_obj[i]['numInstance']):
                                    if (i==primary_object and m==primary_instance):
                                        glow = 2
                                    elif (i==secondary_object and m==secondary_instance):
                                        glow = 1
                                    else:
                                        glow = 0
    ...
    def overlay_deformable_person(self, bg_img, img_pad_num, cur_obj, z_scale, glow):
        ...
        def_person = Image.new(bg_img.mode, bg_size, 0)
        ...
        for partIdx in range(0, num_parts):
            ...
            def_person.paste(part_tf, (0,0), part_tf)

        if(glow == 1):
            def_person = makeShadow(def_person, [0, 0])
        elif(glow == 2):
            def_person = makeShadow(def_person, [0, 0], 'OrangeRed')

        bg_img.paste(def_person, (0, 0), def_person)

    def overlay_nondeformable_person(self, bg_img, img_pad_num, cur_obj, z_scale, glow):
        ...
        self.overlay_nondeformable_obj(bg_img, img_pad_num, cur_obj, z_scale, glow)

    def overlay_nondeformable_obj(self, bg_img, img_pad_num, cur_obj, z_scale, glow):
        ...
        if (cur_obj['flip'] == 0):
            if glow == 1:
                resized = makeShadow(resized, [0,0])
            elif(glow == 2):
                resized = makeShadow(resized, [0, 0], 'OrangeRed')
            bg_img.paste(resized, offset, resized)
        else:
            flipped = resized.transpose(Image.FLIP_LEFT_RIGHT)
            if glow == 1:
                flipped = makeShadow(flipped, [0,0])
            elif(glow == 2):
                flipped = makeShadow(flipped, [0, 0], 'OrangeRed')
