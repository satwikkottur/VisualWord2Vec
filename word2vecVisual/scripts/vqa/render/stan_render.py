...
import json
import sys
sys.path.append("/home/linxiao/test/mturk_bot/code/lib/lib/python2.7/site-packages/");
												...
												for i in range(0, len(cur_avail_obj)):
																if (cur_avail_obj[i]['instance'][0]['depth0'] == k):
																				for m in range(0, cur_avail_obj[i]['numInstance']):
																								if ((i == primary_object and m == primary_instance) or 
																																(i==secondary_object and m == secondary_instance)):
																												glow = 1
																		else:
																						glow = 0
...
def overlay_deformable_person(self, bg_img, img_pad_num, cur_obj, z_scale, glow):
				...
				for partIdx in range(0, num_parts):
						...
						#add glow if needed
						if glow == 1:
										part_tf = makeShadow(part_tf, [2, 2])
