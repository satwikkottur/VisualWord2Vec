...
char save_vocab_file[MAX_STRING], read_vocab_file[MAX_STRING], read_cnn_file[MAX_STRING];
struct vocab_word *vocab;
int binary = 0, cbow = 1, debug_mode = 2, window = 5, min_count = 5, num_threads = 12, min_reduce = 1;
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100, sentence_vectors = 0, global_image = 0;
long long layer1_image_size = 4096, image_size = 1000;
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0, classes = 0;
real alpha = 0.025, starting_alpha, sample = 1e-3, factor_image = 0.1;
real *syn0, *syn1, *syn1neg, *expTable, *syn0_im;
clock_t start;
...
void ReadCNNFeature() {
  FILE *fin  = fopen(read_cnn_file, "rb");
  if (fin == NULL) {
    printf("CNN file for images not found\n");
    exit(1);
  }
  real val;
  int a, b;
  for(a = 0; a < image_size; a++) {
    for (b = 0 ; b < layer1_image_size; b++) {
      fscanf(fin,"%f,",&val);
      // Normalize cnn feature by layer1_size
      syn0_im[a * layer1_image_size + b] = val / layer1_size;
    }
  }
  fclose(fin);
}
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\t-global-image <int>\n");
    printf("\t\tUse image corresponding to sentences to train both word vector and image representation; default is 0\n");
    printf("\t-read-cnn <file>\n");
    printf("\t\tUse image as global context, read cnn file\n");
    printf("\t-image-size <int>\n");
    printf("\t\tTraining image number\n");
    printf("\t-factor-image <float>\n");
    printf("\t\tFactor of image feature against word\n");
    printf("\nExamples:\n");
  ...
  if ((i = ArgPos((char *)"-global-image", argc, argv)) > 0) global_image = atoi(argv[i+1]);
  if ((i = ArgPos((char *)"-read-cnn", argc, argv)) > 0) strcpy(read_cnn_file, argv[i + 1]); 
  if ((i = ArgPos((char *)"-image-size", argc, argv)) > 0) image_size = atoi(argv[i+1]);
  if ((i = ArgPos((char *)"-factor-image", argc, argv)) > 0) factor_image = atof(argv[i+1]);
